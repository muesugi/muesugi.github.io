...
export default class UIPortfolioPage extends React.Component {
  ...
  componentWillReceiveProps(newProps) {
    ...
    if (newParams.filter && newParams.filter != this.props.filter) {
      this.setState({
        filterSkill: newParams.filter
          ? decodeURIComponent(newParams.filter)
          : "",
      });
    }
    if (newParams.anchor && newParams.anchor != this.props.anchor) {
      this.setState({
        anchor: newParams.anchor ? decodeURIComponent(newParams.anchor) : "",
      });
    }
    this.scrollToAnchor();
  }
  onFilterSkillChange(newValue) {
    if (!newValue) {
      ...
      history.pushState(
        null,
        null,
        "#/ui-portfolio/?filter=" + encodeURIComponent(newValue)
      );
